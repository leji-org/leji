...
test('run viewer generates the viewer and prints the serve hint', async () => {
   ...
   assert.ok(fs.existsSync(path.join(dir, '.leji', 'viewer', '_sidebar.md')));
});
...
const CIRCLE_CONFIG = `# generated by leji ci (managed) v2
version: 2.1
jobs:
  leji-validate:
    docker:
...
const AZURE_PIPELINE = `# generated by leji ci (managed) v2
trigger:
  - main
pool:
...
test('ci --provider gitlab: a standing entry that is not a regular file is refused, never merged', async () => {
   ...
   const dir = await seededCiDir('leji-ci-gl-kind-');
   fs.mkdirSync(path.join(dir, 'inside-dir'));
   fs.symlinkSync(path.join(dir, 'inside-dir'), path.join(dir, '.gitlab-ci.yml'));
   const r = await runInProcess(['ci', '--root', dir, '--provider', 'gitlab']);
   assert.equal(r.code, 2);
   assert.match(r.err, /refusing to write through a symlink that escapes the target/);
});

test('ci --provider circleci: creates when absent, re-runs clean, hands a snippet for a foreign file', async () => {
   ...
   // user never wrote.
   const again = await runInProcess(['ci', '--root', dir, '--provider', 'circleci', '--json']);
   assert.equal(again.code, 0);
   assert.equal(JSON.parse(again.out).action, 'unchanged');
   assert.equal(fs.readFileSync(cc, 'utf8'), CIRCLE_CONFIG, 'idempotent byte-for-byte');

   // Someone else's config: never modified, and the snippet comes back to add by hand.
   const foreign = await seededCiDir('leji-ci-cc-foreign-');
   const fcc = path.join(foreign, '.circleci', 'config.yml');
   fs.mkdirSync(path.dirname(fcc), { recursive: true });
   fs.writeFileSync(fcc, 'version: 2.1\njobs:\n  mine: {}\n');
   const manual = await runInProcess(['ci', '--root', foreign, '--provider', 'circleci', '--json']);
   ...
   assert.equal(fs.readFileSync(fcc, 'utf8'), 'version: 2.1\njobs:\n  mine: {}\n', 'foreign config untouched');
});
...
test('ci: a dangling target symlink is refused, never written through', async () => {
   // Every arm decided presence with `existsSync`, which follows symlinks: a dangling
   ...
   for (const [provider, targetRel] of [
      ...
      const dir = await seededCiDir('leji-ci-dangling-');
      const target = path.join(dir, targetRel);
      fs.mkdirSync(path.dirname(target), { recursive: true });
      fs.symlinkSync('never-created.yml', target);
      const r = await runInProcess(['ci', '--root', dir, '--provider', provider]);
      assert.equal(r.code, 2, `${provider}: dangling target refused`);
      assert.match(r.err, /refusing to write through a symlink that escapes the target/);
      assert.equal(
         fs.existsSync(path.join(path.dirname(target), 'never-created.yml')),
         false,
         `${provider}: the dangling link's destination is never created`,
      );
      assert.ok(fs.lstatSync(target).isSymbolicLink(), `${provider}: the planted link is left exactly as it was`);
   }
});
...
test('agent: writing the default binding prints the selects-vs-loads guidance (human and JSON); other keys and re-runs do not', async () => {
   const guidance =
   ...
   const seed = async (prefix: string): Promise<string> => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), prefix));
      await runInProcess(['init', '--dir', dir, '--yes', '--name', 'demo']);
      return dir;
   };
   // human output: the guidance follows the success lines, byte-exact
   const dir = await seed('leji-run-agent-');
   const human = await runInProcess(['agent', '--name', 'default', '--root', dir]);
   assert.equal(human.code, 0, human.out + human.err);
   assert.match(human.out, /Bound agent "default"/);
   assert.equal(human.out.split('\n').at(-1), guidance);
   // written-only: a re-run binds nothing and stays terse, in both modes
   const again = await runInProcess(['agent', '--name', 'default', '--root', dir]);
   assert.equal(again.code, 0, again.out + again.err);
   assert.ok(!again.out.includes('selects a role profile'), 'no guidance when nothing was bound');
   const againJson = await runInProcess(['agent', '--name', 'default', '--json', '--root', dir]);
   assert.equal(JSON.parse(againJson.out).note, undefined);
   // JSON mode carries the same sentence in `note` (the CI activation-note pattern)
   const json = await runInProcess(['agent', '--name', 'default', '--json', '--root', await seed('leji-run-agent-j-')]);
   assert.equal(json.code, 0, json.out + json.err);
   assert.equal(JSON.parse(json.out).note, guidance);
   // any other binding stays quiet, in both modes
   const other = await runInProcess(['agent', '--name', 'reviewer', '--root', dir]);
   assert.equal(other.code, 0, other.out + other.err);
   assert.ok(!other.out.includes('selects a role profile'), 'no guidance for a non-default key');
   const otherJson = await runInProcess(['agent', '--name', 'thought-partner', '--json', '--root', dir]);
   assert.equal(otherJson.code, 0, otherJson.out + otherJson.err);
   assert.equal(JSON.parse(otherJson.out).note, undefined);
});
...
test('init interactive with a "." context root yields context/ (not ./ reject, not .context/)', async () => {
   ...
   assert.match(brief, /\.leji\/work\/onboarding-brief\.md/, 'brief references .leji/work/onboarding-brief.md');
...
// --- the declaration step (init/adopt) --------------------------------------
// The dependency offer is the one place the CLI can run a program that writes to
// the repository, so every mode is pinned here: what is printed, that nothing runs
// without a terminal and a yes, and what the JSON document carries.

/** A planted dependency root: a manifest and its (empty) lockfiles, nothing else. */
function plantRoot(files: Record<string, string>): string {
   const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'leji-dep-proc-'));
   for (const [rel, body] of Object.entries(files)) fs.writeFileSync(path.join(dir, rel), body);
   return dir;
}

const PNPM_ROOT = { 'package.json': '{\n  "name": "demo"\n}\n', 'pnpm-lock.yaml': '' };

test('init --yes prints the declaration block and runs no package manager', async () => {
   const dir = plantRoot(PNPM_ROOT);
   const before = fs.readFileSync(path.join(dir, 'package.json'), 'utf8');
   const r = await runInProcess(['init', '--dir', dir, '--yes', '--name', 'demo-context']);
   assert.equal(r.code, 0);
   assert.match(r.out, /Detected pnpm \(pnpm-lock\.yaml\)\. To declare the Leji CLI as a dev dependency/);
   assert.match(r.out, /\n {3}pnpm add -D @leji-org\/leji/);
   // --yes is not a terminal consent: nothing ran, so the manifest is untouched and
   // no manager wrote a lockfile or a node_modules tree.
   assert.equal(fs.readFileSync(path.join(dir, 'package.json'), 'utf8'), before);
   assert.equal(fs.readFileSync(path.join(dir, 'pnpm-lock.yaml'), 'utf8'), '');
   assert.equal(fs.existsSync(path.join(dir, 'node_modules')), false);
   // The block lands after the write list and before the handoff text.
   assert.ok(r.out.indexOf('Wrote ') < r.out.indexOf('Detected pnpm'), 'block follows the write list');
});

test('init --yes --json is one document carrying the ecosystem, and no block', async () => {
   const dir = plantRoot(PNPM_ROOT);
   const r = await runInProcess(['init', '--dir', dir, '--yes', '--json', '--name', 'demo-context']);
   assert.equal(r.code, 0);
   const doc = JSON.parse(r.out);
   assert.deepEqual(Object.keys(doc), ['command', 'ok', 'findings', 'summary', 'written', 'ecosystem']);
   assert.equal(doc.command, 'init');
   assert.equal(doc.ok, true);
   assert.ok(doc.written.includes('leji.json'));
   assert.equal(doc.ecosystem.selected.manager, 'pnpm');
   assert.deepEqual(doc.ecosystem.selected.add, ['pnpm', 'add', '-D', '@leji-org/leji']);
   assert.equal(doc.ecosystem.reason, null);
   // Single document: the human block and the handoff prose stay out of it.
   assert.doesNotMatch(r.out, /To declare the Leji CLI/);
   assert.doesNotMatch(r.out, /The scaffold is in place/);
});

test('init --dry-run prints the block after the plan and writes nothing', async () => {
   const dir = plantRoot({ 'pyproject.toml': '[project]\nname = "demo"\nversion = "0.1.0"\n', 'uv.lock': '' });
   const r = await runInProcess(['init', '--dir', dir, '--yes', '--dry-run', '--name', 'demo-context']);
   assert.equal(r.code, 0);
   assert.match(r.out, /No files written \(--dry-run\)/);
   assert.match(r.out, /Detected uv \(uv\.lock\)\./);
   assert.match(r.out, /\n {3}uv add --dev leji/);
   assert.ok(r.out.indexOf('No files written') < r.out.indexOf('Detected uv'), 'block follows the plan render');
   assert.equal(fs.existsSync(path.join(dir, 'leji.json')), false);
});

test('init --dry-run --json is one document with the ecosystem and no writes', async () => {
   const dir = plantRoot({ 'go.mod': 'module example.com/demo\n\ngo 1.24.0\n' });
   const r = await runInProcess(['init', '--dir', dir, '--yes', '--dry-run', '--json', '--name', 'demo-context']);
   assert.equal(r.code, 0);
   const doc = JSON.parse(r.out);
   assert.equal(doc.dryRun, true);
   assert.deepEqual(doc.written, []);
   assert.equal(doc.ecosystem.selected.manager, 'go');
   assert.deepEqual(doc.ecosystem.selected.add, [
      'go',
      'get',
      '-tool',
      'github.com/leji-org/leji/packages/sdk-go/cmd/leji@latest',
   ]);
   assert.equal(fs.existsSync(path.join(dir, 'leji.json')), false);
});

test('init on a root with no manifest still prints the per-person install block', async () => {
   const dir = plantRoot({});
   const r = await runInProcess(['init', '--dir', dir, '--yes', '--name', 'demo-context']);
   assert.equal(r.code, 0);
   assert.match(r.out, /No package\.json, pyproject\.toml or go\.mod here/);
   assert.match(r.out, /https:\/\/leji\.org\/quickstart\//);
});

test('init on an ambiguous root offers one command per candidate and runs nothing', async () => {
   const dir = plantRoot({ 'package.json': '{}\n', 'package-lock.json': '', 'yarn.lock': '' });
   const r = await runInProcess(['init', '--dir', dir, '--yes', '--name', 'demo-context']);
   assert.equal(r.code, 0);
   assert.match(r.out, /leji will not guess the package manager/);
   assert.match(r.out, /\n {3}npm i -D @leji-org\/leji/);
   assert.match(r.out, /\n {3}yarn add -D @leji-org\/leji/);
});

test('adopt --yes prints the block; adopt --json is one document', async () => {
   const dir = plantRoot({ ...PNPM_ROOT });
   fs.mkdirSync(path.join(dir, 'docs'));
   fs.writeFileSync(path.join(dir, 'docs', 'README.md'), '# Docs\n');
   const r = await runInProcess(['adopt', '--dir', dir, '--yes']);
   assert.equal(r.code, 0);
   assert.match(r.out, /Detected pnpm \(pnpm-lock\.yaml\)\./);

   const dir2 = plantRoot({ ...PNPM_ROOT });
   fs.mkdirSync(path.join(dir2, 'docs'));
   fs.writeFileSync(path.join(dir2, 'docs', 'README.md'), '# Docs\n');
   const j = await runInProcess(['adopt', '--dir', dir2, '--yes', '--json']);
   assert.equal(j.code, 0);
   const doc = JSON.parse(j.out);
   assert.equal(doc.command, 'adopt');
   assert.equal(doc.ecosystem.selected.manager, 'pnpm');
   assert.doesNotMatch(j.out, /To declare the Leji CLI/);
});
