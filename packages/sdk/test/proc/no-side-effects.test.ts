...
// Subprocess runner: reserved for the one smoke test that proves the installed
// bin entrypoint (not just run()) honors a meta-flag without writing.
function runCliProc(args: string[], cwd: string): Promise<CliResult> {
...
// In-process runner: chdir into the sandbox so the default --root "." resolves
// there exactly like the subprocess (a regressed write-intent command would still
// scaffold into the snapshotted dir), capture console output, and call run()
// directly. No node cold-start, so it does not contend under the concurrent suite.
// node --test isolates each file in its own process and runs its tests
// sequentially, so the transient chdir cannot race another test.
...
// Smoke (real process): prove the installed bin entrypoint, not just run(), honors
// a meta-flag without writing. Guards the original `leji adopt --help` bug where a
// write-intent command ran instead of printing help.
test('the bin honors "adopt --help" without writing (real process)', async () => {
   const dir = sandboxEmpty();
   const before = snapshot(dir);
   const r = await runCliProc(['adopt', '--help'], dir);
   assert.equal(r.code, 0, r.stderr);
   assert.match(r.stdout, /Usage: leji/);
   assert.deepEqual(snapshot(dir), before, 'adopt --help wrote files');
});
