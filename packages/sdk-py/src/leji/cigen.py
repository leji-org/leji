"""The generated CI job and the pre-commit hook body.
...
Transcribed from the TypeScript reference (``packages/sdk/src/commands/init.ts``).
"""
...
from .ecosystem import DEP_NAME, EcosystemReport, manager_runner_argv

# Every provider `leji ci` generates for, in a fixed order.
CI_PROVIDERS = ["github", "gitlab", "circleci", "azure"]


@dataclass(frozen=True)
class CiManagerCell:
    """One package manager's CI facts. ``pip_bootstrap`` names a tool that has to be
    installed with pip wherever the provider offers no dedicated setup action;
    ``unpinned`` names a bootstrap tool the job installs unpinned, disclosed in one
    comment line."""
...
# it comes from the detection report, which owns the one runner table. Insertion
# order is the enumeration order the goldens depend on.
CI_MANAGERS: dict[str, CiManagerCell] = {
    "npm": CiManagerCell(runtime="node", install="npm ci"),
    "pnpm": CiManagerCell(
        runtime="node", install="corepack enable && pnpm install --frozen-lockfile"
    ),
    "yarn": CiManagerCell(
        runtime="node", install="corepack enable && yarn install --frozen-lockfile"
    ),
    "bun": CiManagerCell(runtime="bun", install="bun install --frozen-lockfile"),
    "uv": CiManagerCell(runtime="python", install="uv sync --locked", pip_bootstrap="uv"),
    "poetry": CiManagerCell(
        runtime="python", install="pip install poetry && poetry install", unpinned="poetry"
    ),
    "pdm": CiManagerCell(
        runtime="python", install="pip install pdm && pdm install", unpinned="pdm"
    ),
    "pipenv": CiManagerCell(
        runtime="python", install="pip install pipenv && pipenv install --dev", unpinned="pipenv"
    ),
    "go": CiManagerCell(runtime="go", install="go mod download"),
}
...
CI_FALLBACK_NODE = ["npx", "-y", f"{DEP_NAME}@1"]
CI_FALLBACK_PY_INSTALL = "pip install 'leji>=1,<2'"
CI_FALLBACK_GO_INSTALL = "go install github.com/leji-org/leji/packages/sdk-go/cmd/leji@latest"
...
def resolve_ci_job(report: EcosystemReport, provider: str) -> CiJob:
    ...
    if (
        ...
        and selected.direct_declared
        and selected.lock_evidenced
        and selected.runner
    ):
        ...
        uv_action = provider == "github" and cell.pip_bootstrap == "uv"
        bootstrap = cell.pip_bootstrap if cell.pip_bootstrap and not uv_action else ""
        install = f"pip install {bootstrap} && {cell.install}" if bootstrap else cell.install
        return CiJob(
            runtime=cell.runtime,
            install=[install],
            runner=list(selected.runner),
            unpinned=cell.unpinned or bootstrap,
            uv_action=uv_action,
            local=True,
        )
    ecosystem = report.all[0].ecosystem if len(report.all) == 1 else None
    if ecosystem == "python":
        return CiJob(runtime="python", install=[CI_FALLBACK_PY_INSTALL], runner=["leji"])
    if ecosystem == "go":
        return CiJob(runtime="go", install=[CI_FALLBACK_GO_INSTALL], runner=["leji"])
    # Node, several ecosystems, and none alike: the job that needs no package manager.
    return CiJob(runtime="node", install=[], runner=list(CI_FALLBACK_NODE))
...
CI_GENERATOR_VERSION = "2"
CI_MARKER = f"# generated by leji ci (managed) v{CI_GENERATOR_VERSION}"

GITLAB_MARKER_START = "# >>> leji ci (managed) >>>"
GITLAB_MARKER_END = "# <<< leji ci (managed) <<<"


def _unpinned_note(job: CiJob) -> str:
    """The one disclosure line for a job that installs a bootstrap tool unpinned."""
    if not job.unpinned:
        return ""
    return f"# {job.unpinned} is installed unpinned here; pin it if your project pins it."


def _github_setup(job: CiJob) -> list[str]:
...
def build_github_workflow(job: CiJob) -> str:
    note = _unpinned_note(job)
    runner = " ".join(job.runner)
    lines = [
        CI_MARKER,
        "name: leji",
        ...
        *[f"      - run: {cmd}" for cmd in job.install],
        f"      - run: {runner} validate",
        f"      - run: {runner} index --check",
    ]
    return "\n".join(lines) + "\n"
...
def build_gitlab_block(job: CiJob) -> str:
    note = _unpinned_note(job)
    runner = " ".join(job.runner)
    # `.pre` is always available. Without an explicit stage GitLab assigns `test`, and
    # a pipeline whose own `stages:` list omits `test` rejects the whole configuration,
    # so the generated job would break an existing pipeline it was merged into.
    lines = [
        GITLAB_MARKER_START,
        "leji-validate:",
        "  stage: .pre",
        f"  image: {_ci_image(job.runtime)}",
        "  script:",
        *([f"    {note}"] if note else []),
        *[f"    - {cmd}" for cmd in job.install],
        f"    - {runner} validate",
        f"    - {runner} index --check",
        GITLAB_MARKER_END,
    ]
    return "\n".join(lines) + "\n"


def _circleci_job(job: CiJob) -> list[str]:
...
def build_circleci_config(job: CiJob) -> str:
    return "\n".join([CI_MARKER, "version: 2.1", *_circleci_job(job)]) + "\n"


def build_circleci_snippet(job: CiJob) -> str:
...
def build_azure_pipeline(job: CiJob) -> str:
    note = _unpinned_note(job)
    runner = " ".join(job.runner)
    install_lines: list[str] = []
    for cmd in job.install:
        install_lines.extend([f"  - script: {cmd}", "    displayName: install"])
    lines = [
        CI_MARKER,
        "trigger:",
        "  - main",
        ...
        *([f"  {note}"] if note else []),
        *install_lines,
        f"  - script: {runner} validate",
        "    displayName: leji validate",
        f"  - script: {runner} index --check",
        "    displayName: leji index --check",
    ]
    return "\n".join(lines) + "\n"


def build_ci_file(provider: str, job: CiJob) -> str:
    """The whole file a provider writes for a job; GitLab's is the block it owns
    inside a shared file."""
    if provider == "github":
        return build_github_workflow(job)
    if provider == "gitlab":
        return build_gitlab_block(job)
    if provider == "circleci":
        return build_circleci_config(job)
    return build_azure_pipeline(job)


def _ci_job_variants(provider: str) -> list[tuple[str, CiJob]]:
    """Every job this generator can produce, in a fixed order: the nine local manager
    cells, then the three ecosystem fallbacks."""
    out: list[tuple[str, CiJob]] = []
    for manager, cell in CI_MANAGERS.items():
        uv_action = provider == "github" and cell.pip_bootstrap == "uv"
        bootstrap = cell.pip_bootstrap if cell.pip_bootstrap and not uv_action else ""
        install = f"pip install {bootstrap} && {cell.install}" if bootstrap else cell.install
        out.append(
            (
                ...
                CiJob(
                    runtime=cell.runtime,
                    install=[install],
                    runner=manager_runner_argv(manager) or ["leji"],
                    unpinned=cell.unpinned or bootstrap,
                    uv_action=uv_action,
                    local=True,
                ),
    ...
    out.append(("node-fallback", CiJob(runtime="node", install=[], runner=list(CI_FALLBACK_NODE))))
    out.append(
        (
            "python-fallback",
            CiJob(runtime="python", install=[CI_FALLBACK_PY_INSTALL], runner=["leji"]),
        )
    )
    out.append(
        ("go-fallback", CiJob(runtime="go", install=[CI_FALLBACK_GO_INSTALL], runner=["leji"]))
    )
    return out


def ci_variants() -> list[tuple[str, str, str]]:
    """Every generated artifact of the CURRENT generator: (provider, key, bytes)."""
    out: list[tuple[str, str, str]] = []
    for provider in CI_PROVIDERS:
        for key, job in _ci_job_variants(provider):
            out.append((provider, key, build_ci_file(provider, job)))
    return out


# Digests of every whole file this generator has ever written, so a file leji created
# in an EARLIER release is still recognized as its own and upgraded rather than
# abandoned. Appended at each release; the marker line carries the generator version
# that wrote a file, and these digests carry the ones that predate it.
#
# Keyed by provider, and consulted only for the provider whose path is being written:
# the same bytes are leji's workflow at .github/workflows/leji.yml and somebody
# else's file at .azure-pipelines/leji.yml.
#
...
def is_leji_generated(provider: str, text: str) -> bool:
    ...
    return digest in KNOWN_GENERATED.get(provider, [])


# --- the hook -------------------------------------------------------------

HOOK_MARKER = "# leji pre-commit (managed)"
HUSKY_MARKER_START = "# >>> leji hooks (managed) >>>"
HUSKY_MARKER_END = "# <<< leji hooks (managed) <<<"
...
def _sh_command(runner: list[str]) -> str:
    """The runner argv as one quoted command prefix."""
    return " ".join(sh_quote(word) for word in runner)


def _hook_gates(runner: list[str]) -> str:
    ...
    leji = _sh_command(runner)
    return (
        f"{leji} validate || exit 1\n"
        f"{leji} index --check || {{\n"
...
def hook_body(runner: list[str]) -> str:
    ...
    return (
        ...
        f"{_hook_gates(runner)}"
    )


def husky_block(runner: list[str]) -> str:
    """The same two gates ``hook_body`` runs, wrapped in markers so the block can be
    merged into a husky repo's hand-authored ``.husky/pre-commit`` without touching its
    rest."""
    return f"{HUSKY_MARKER_START}\n{_hook_gates(runner)}{HUSKY_MARKER_END}\n"
