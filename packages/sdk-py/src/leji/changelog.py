...
def seed_changelog_if_missing(root: str, manifest: Manifest) -> Optional[str]:
    ...
    check, because ``Path.is_file()`` follows symlinks: a dangling link at the
    changelog name reads as absent and the seed would be created at the link's
    destination. The exclusive create judges the ORIGINAL entry, so any standing entry
    is the same already-present no-op an existing changelog is."""
    ...
    if not write_file_guarded(
        guard_root(root), str(abs_path), None, serialize_changelog(log), exclusive=True
    ).ok:
        return None
    return rel
...
def compact_changelog(
    ...
    read = verified_target_read(root_real, str(Path(root) / rel), None)
        ...
        return CompactResult(
            findings=[
                Finding(
                    "artifact-parse",
                    "error",
                    f"artifact {rel} resolves outside the layer root",
                    rel,
                )
            ],
            folded=0,
    ...
    try:
        ...
        return CompactResult(
            findings=[Finding("artifact-parse", "error", f"invalid JSON: {e}", rel)],
            folded=0,
            kept=0,
            path=rel,
        )
    ...
    abs_path = Path(root) / rel
    if not write_file_guarded(root_real, str(abs_path), None, serialize_changelog(nxt)).ok:
        return CompactResult(
            findings=[
                Finding(
                    "artifact-parse",
