...
The tree above is a drawing of the finished setup, not of this directory. `app-payments/context/` marks where the submodule mounts in a real consumer repository; nothing is checked in here, so there is no `context/` to browse under `app-payments/`.

What to notice:

- The docs-only submodule (`app-payments/context/`, [pattern 2](../../spec/distribution.md)) is a leaf: no build or runtime step touches it, so a stale pin degrades knowledge, never the build.
