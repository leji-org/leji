...
Imperative instructions ("do this, in this format, for this vendor file") have to be re-encoded for every agent host, every task, every new hire. Durable intent (what things mean, what must hold, why it is so) is written once and derived from, by any host and for any task. That's why the context layer's categories are intent-shaped (domain meaning, system invariants, governance rules, decision rationale) rather than task-shaped. Task envelopes may come in a future version, extracted from live practice rather than designed in advance; 1.0 deliberately standardizes the smaller surface.
...
A real repository's documentation is two interleaved corpora with different truth models. Intent (glossaries, invariants, conventions, guardrails) must be *kept* true: when reality moves, the document is corrected, and freshness horizons make neglect visible. Records (statuses, assessments, ledgers, readouts, archives) are *born* true within a boundary: later state supersedes them, and their date is their whole currency. Forcing the second corpus under the first's mechanism fails both ways: either records carry review promises nobody can honor, or the operational majority of the repository is exiled from governance entirely.

So Leji governs both, distinctly. Records are indexed, reviewed, owned, and routed like intent; what changes is the reader's contract (dated evidence, never current truth) and the freshness mechanism (horizons apply to intent alone). The five categories stay topic axes; the kind is orthogonal, because a system assessment is *about* the system and *true as of its date* at once. And the spec stops where honesty stops: it never certifies which record is "the latest," because nothing in 1.0 declares record series or their ordering; a reader judges recency from the dates the index surfaces. Decisions were the tell all along: the spec's own decision records, dated and append-only and never stale, are the record pattern generalized.

## Why this is not a wiki

Wikis rot because nothing forces them current. The context layer has 3 forcing functions a wiki lacks: agents read it on every task, so wrong context produces wrong output someone feels the same day; changes ride code review, so there's no separate process to forget; and mechanical drift (a stale index, a rewound changelog, a broken profile) is a check that fails, which at `governed` runs in CI on every change. Maintenance is one delta at a time, riding work that's happening anyway.
