...
- The SDK runs locally. It collects no telemetry and reports nothing back; validation, scaffolding, and conformance work fully offline. The only network access is in the federation commands a user invokes deliberately (`mounts hydrate --fetch`, `conformance --federation=verify`), which contact the declared source and nothing else.
